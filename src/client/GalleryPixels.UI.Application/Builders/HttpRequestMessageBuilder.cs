...
namespace GalleryPixels.UI.Application.Builders;
...
public class HttpRequestMessageBuilder
{
    ...
    {
        if (queries is null) return this;
    ...
    public HttpRequestMessageBuilder WithBody<TValue>(TValue value, JsonSerializerOptions? options = null) where TValue : notnull
    {
        if (_method == HttpMethod.Get) throw new ArgumentException("Can not add a body to a GET request");
    ...
    public HttpRequestMessage Build()
    {
        ...
        var request = new HttpRequestMessage(_method, _endpoint + (queryParameters.Count > 0 ? "?" + queryParameters : string.Empty)) { Content = _content };
